...
async def list_markdown_files(
    ...
    min_date: datetime = Query(
        None, description="Filter by minimum modification date."
    ),
    max_date: datetime = Query(
        None, description="Filter by maximum modification date."
    ),
    path_query: str = Query(None, description="Filter by file path."),
):
    logger.info(
        f"Listing markdown files for repository {owner}/{repo}. Ref: {ref}, Filters: {locals()}"
    )
    try:
        ...
        logger.info(
            f"Successfully listed markdown files. Total found: {len(filtered_files)}"
        )
...
async def get_markdown_content(
    ...
    logger.info(
        f"Fetching content of markdown file: {file_path} from repository {owner}/{repo}, Ref: {ref}"
    )
    try:
        async with httpx.AsyncClient() as client:
