...
async def make_github_request(client: httpx.AsyncClient, url: str) -> httpx.Response:
    for _ in range(len(Config.tokens)):
        ...
        logger.warning("Rate limit atingido para o token atual. Alternando para o próximo token.")
...
async def fetch_markdown_files(client: httpx.AsyncClient, owner: str, repo: str, path: str = "", ref: str = "main") -> List[Dict]:
    ...
    for file in files:
        if file["type"] == "dir":
            ...
            md_files.extend(await fetch_markdown_files(client, owner, repo, file["path"], ref))
...
async def fetch_file_content(client: httpx.AsyncClient, owner: str, repo: str, file_path: str, ref: str = "main") -> str:
...
async def search_markdown_content(
    ...
    logger.info(f"Pesquisando conteúdo em arquivos Markdown no repositório {owner}/{repo}. Query: {query}, Ref: {ref}")
    ...
    for file in md_files:
        try:
            content = await fetch_file_content(client, owner, repo, file["path"], ref=ref)
